...
总体来说，我的三月就这么过去了，首先我还是觉得非常幸运，面试通过了字节的后台开发岗位（3+1），pingcap一面和wxg一面，五次面试无一败绩。成绩斐然。然后代码统计量是97k行代码（其中刷题代码占80%），利用较为扎实的C++基础，使用35h在Java零基础的情况下完成了Java实验：https://github.com/sohardforaname/UcchiSearchEngine 并通过全部测试， 平均每小时编写50行代码，这个速度还是相对来说令人满意的。
...
帮新生出题忙了一天，但是感到了一些快乐。人在自己做的事情能影响别人（往积极的方向）的时候，会感到开心吧？但是时间和个人能力的原因，让我没能出完题。不过到现在为止题已经出好了，下一次就能用了。

然后今天其实啥都没干，除了出题之外。但是获得了很多意外的收获。我感到很开心。其实如果不一直盯着某些事情看的话，阴云也会散开不少，阳光也将会悄悄洒满大地。希望自己不要迷茫于此。把自己想干的事情认真的完成，这才是对自己最好的回报。

然后明天应该就是日经写题了。刷codeforces，现在博客园又能用了，blog持续更新中。然后晚上优化一下搜索引擎，写完CompressedTrie。

昨晚其实挺晚睡的，想了很久。还是非常感谢acm，它是我一直以来难受时的支持。也是我灵活解决问题的启发，虽然它和工程相差甚远。但是严谨的思维和逻辑能力奠定了我作为一个工程师的基石。

说了这么多，希望自己能在5月的银川Regional拿银吧。我这仅有一次的大学生活，还是很想去一次ec-final的啊。加油！

## 2021.4.7

太草了，wxg二面光速挂我，只能说应该是hc满了+我答得不行。

说到这个东西，我觉得我也确实是需要检讨一下自己最近的一些神必的事情了。首先是自从字节过了之后，我就开始极度放松，原有的面试知识点基本已经忘记了。自己沉迷于虚幻的成就之中，殊不知命运的安排悄然降临。

然后就答得稀烂，没啥好说的，挂也太正常了。只能说，勿在浮沙筑高台。

然后就是一天一度的计划了。

首先是作息，我们标准睡觉时间是1点，9点多起，睡觉时间过长，必须尝试减少睡觉时间至7.5h/d（4.8早上完成了，继续保持）。

然后是学习计划：

首先就是计算机网络自顶向下必须再次通读一遍，完成middle和hard习题。现代操作系统通读一遍，完成middle和hard习题。

然后是数据库系统实现的通读和middle和hard习题（当然是要装个redis和MySQL自己玩玩）

然后是购入现代编译器实现。结合龙书学习编译原理。

在5.15之前恢复一天一套cf+两个中档题的题量。同步学习数据库和编译原理。5.15之后通读自顶向下方法和现代操作系统。

然后对自己的项目做一个全方位的深挖，blog持续更新中。

加油！秋招再战wxg！

## 2021.4.8

最后还是鼓起勇气获得了新交的故友。其实自己还是有一点点遗憾吧，但是现在也已经是最好的结果了。昨晚和故友聊了很多东西，算是把之前所有的误会都解决了，聊完之后和苏轼共游赤壁。记录下来与君共勉。

苏子曰：“客亦知夫水与月乎？逝者如斯，而未尝往也。盈虚者如彼，而卒莫消长也。盖将自其变者而观之，则天地曾不能以一瞬。自其不变者而观之，而物与我皆无尽也，而又何羡乎？且夫天地之间，物各有主，苟非吾之所有，虽一毫而莫取，而江上之清风，与山间之明月，耳得之而为声，目遇之而成色，取之不尽，用之不竭，是造物者之无尽藏也，而吾与子之所共适。”

现在所有的历史遗留问题（除了我菜之外），都解决了。

然后就是自己的梦想了。

目前自己的想法就是银川拿银吧（草地名暗示拿银有手就行）。明年可能要重组队伍去ec-final。

好，开始codeforces刷题和总结。

by the way，后天大概是我大学最后一场csp了。冲就完事了。

## 2020.4.11

之前发生的种种事情，占用了我大量的时间，让我的计划被严重打乱，同时，不断增加的任务让我喘不过气来。让我迷失。
因此，今晚我需要尝试梳理我在大学里面想做但是未能做到的事情，结合自己现在的课程进行调整，以提升自己的效率，然后我们对这些天出现的事情进行一次反思。

目前我最突出的矛盾就是时间的缺少与事情增加引起的完成度减少的和计划被严重冲击的矛盾，因此，通过梳理和整合计划，和现在正在学习的课程结合，以提高效率。

当然这个主题也是以日记的形式拆成几篇文章。

第一项就是，先回忆一下我的计划，从大学刚开始进来，到现在为止的计划：

-   光羽星荧光改造和重构计划，wota艺荧光棒开发计划。（嵌入式开发）
-   Problem Manager重构计划。（数据库相关）
-   与LC-3虚拟机配套的编译工具链（lexer ，parser与code optimizer，generator，linker）。(编译原理，计算机系统结构相关)
-   GEMM的进一步优化计划。（底层与算法优化）
-   FastIO的进一步优化计划。（底层与算法优化）
-   RustLuaJIT。（编译原理相关）
-   AyasonDB。（数据库相关）
-   pingcap小作业重构与优化计划。（底层与算法优化）
-   RustLeveldb计划。（Rust语言的完整项目）
-   ICPC YinChuan Regional Ag -> EC-final。（竞赛计划）
-   paper上的算法的实现：（算法实现）
    -   Double Array Trie Aho-Corasick Automaton。
    -   O(n) String split to Parlindrome concentration.
    -   parallel Aho-Corasick Automaton implementation.
    -   Two-way and Wu Manber algorithm.

显然有些项目到现在已经没有继续维护的必要了，有些项目的优先级需要调低。有些项目是需要做下去的。

首先考虑哪些项目需要继续维护下去。可以考虑自己实习需要的和和自己课程相关的，且重复性不能太大：

RustLuaJIT，RustLeveldb，paper算法实现。

再次考虑哪些项目可以先暂缓维护：

FastIO，GEMM(OpenBLAS)，AyasonDB，pingcap面试小项目重构计划。

剩下的项目因为年代久远并且没有维护的意义，遂归档。

当然ICPC YinChuan Regional计划是必须坚持完成的竞赛计划。

另外：对于已经归档和暂缓的计划，并不是说它们已经没用，而是我现在把它的优先级调低而已。或者已经找到了替代手段：

LC-3工具链计划：直接通过刷完csapp完成。

GEMM和FastIO的优化力度基本达到我现有知识的瓶颈，所以需要先读论文和优秀开源项目源码（V8，FastJson, RapidJson），再行优化。

未完待续。

## 2021.4.18

过去一周都比较忙，所以没有继续写下去，在接下来的日记中会继续。
...
组内讨论决定了小组的课设是在CPU上运行kNN算法，然后装好了mips-gcc，最后发现编译出来的代码相比自己写的有点难懂，遂放弃，决定手写mips汇编。

### 4.14

写完了Standard ML的第二次实验，给Java课设加入了查找连词的新功能。

### 4.15

开始看HIT的数据库慕课，计划两到三周看完。
...
周六打了一场训练赛，NWERC2020，我队一共出了6题，达到拿银水平。然后给新生出了一套傻逼码农题。并且T3还是错题，修锅一万年。我属实服了我自己。

### 4.18

摸鱼，用Standard ML重写Java课设。

## 2021.4.20

上一篇日记连流水账都不如，那自然是不行的。

因为之前面试wxg的时候发现自己严重的基础知识漏洞，所以我没得办法就只能多线程补回来，把这些课的HIT的慕课都看看，比如说5月之前完成HIT数据库慕课上。然后5月看OS慕课，5.15比赛完后回来看计网慕课。

训练计划暂定一周两场Regional+补题，codeforces的正赛不缺勤。补题时间全部安排在早上智商最高的时候。

然后最近终于学到了函数式语言的更深入的东西了：Monad，high-order function和currying function。这些函数设计方法是我在命令式编程中闻所未闻的（但是在C++中要复刻它们也并非难事，比如说柯里化代码如下）。
...
int main() {
	shared_ptr<list<int>> ptr(new list<int>({ 1, 2, 3, 5 }));
	doublefunc(*ptr);
	printfunc(*ptr);
	return 0;
}
...
为了继续深入学习函数式语言在更大规模的项目的表达能力和开发效率，我就决定了用Standard ML重写我的Java实验：UcchiSearchEngine（https://github.com/sohardforaname/UcchiSearchEngine）

然后编译原理实验也将会在这几天正式启动，RustLuaJIT。

时间不多了，只能惜时如金了。

## 2021.4.22

今天晚上Standard ML课最后一次实验课结束了，因为和我一起回寝室的小伙伴有事情鸽了我，所以就趁着这个时间构思一下编译原理实验的设计。

我校的编译原理实验是完成一门教学语言decaf的编译器设计，但是decaf说实话，写了就写了，根本没什么意义。所以我选择了Lua。并使用Rust语言实现。但是decaf的整体设计也得先读懂，不然贸然上手LuaJIT一定会出事情。

首先看了编译原理实验的实验安排，整个实验分8周完成。分别是设计文法，词法分析，语法分析，静态检测，语义分析，符号表生成，中间代码生成，中间代码优化，目标代码生成。整个实验的工作量还是比较大的。因为我自己的原因（项目太多，时间太少，略略略）我也只能在这个编译器上分配10d到15d完成。目前的计划是5.8之前完成这个实验。然后在此之间全力准备Regional。得益于我之前Java实验，作业和sml实验的快速完成，我现在唯一要写代码的课，就只有编译原理实验。计划上前两天做Rust语言的语法特性和编程规范的复习和进一步学习，然后前5天完成lexer，parser和syntax checker，后面每两天完成一个实验。最后一天联调联试。

然后慕课课程就很明朗了，李治军老师的os慕课，战德臣老师的数据库慕课。计网慕课就先延后，这是问题不大的。
