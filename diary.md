...
然后就是我的Java老师，辜希武老师，在听了我对Java实验搜索引擎的数据结构提出的优化（使用CompressedTrie，即压缩前缀树）代替哈希表的做法后，对这个数据结构表示出了兴趣。但是显然虽然压缩Trie在具有大量重复前缀的单词集下，是可以显著提高插入，删除，查询性能的，但是输出有序的字典的性能就会相对拖慢，因此我引入了Last-modified机制，以应付大量调用返回字典的过程。但是如果是交替插入和返回有序字典的话，我的CompressedTrie仍然会达到平方级别的时间复杂度。这个我暂时不了解如何改进。
...
然后今天其实啥都没干，除了出题之外。但是获得了很多意外的收获。我感到很开心。其实如果不一直盯着某些神必的事情看的话，阴云也会散开不少，阳光也将会悄悄洒满大地。希望自己不要迷茫于此。把自己想干的事情认真的完成，这才是对自己最好的回报。
...
写完了压缩Trie了。5号搞一下benchmark。看了一下矩阵变换和线性变换的知识，为正式学习光栅化和编写软渲染器做前期准备。

然后找好朋友聊了一下一些神必的事情，我也做出了决定，冲就完事了。

最近特别喜欢范仲淹和苏轼，记录一下范仲淹的《岳阳楼记》的节选（我惊奇的发现我还能背出来，老厉害了）：
...
我真的希望自己能成为“斯人”，现在的我，太像迁客骚人了。

重点提一下，给妹子买礼物送礼物经验get。
